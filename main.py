...
# ============================================================
# POST /encode   — hide any file in a PNG cover image
# POST /decode   — recover a hidden file from a stego PNG
# POST /capacity — max embedding capacity for a cover PNG
#
# PAYLOAD STRATEGY (per file type):
#   text / JSON / XML / HTML / CSV  → zlib compress before encrypt
#   audio / video                   → store RAW  (already compressed)
#   ZIP / 7z / RAR / gzip           → store RAW  (already compressed)
#   JPEG / WEBP / GIF               → store RAW  (already compressed)
#   PNG / BMP / TIFF (images)       → try compress, skip if bigger
#   unknown / octet-stream          → store RAW  (safest default)
#
# PIXEL ORDER: SHA-256(password)[:8] → numpy seed → full permutation.
# Depends only on password + dimensions, never on pixel values.
# Encode and decode always visit the same pixels in the same order.
#
# EMBEDDING: vectorized with np.unpackbits / np.packbits.
# NC-LSBM direction chosen from horizontal neighbor prediction.
#
# HEADER (big-endian integers):
#   0– 3  :  4 B  magic "STEG"
#   4–19  : 16 B  PBKDF2 salt
#  20–31  : 12 B  AES-GCM nonce
#  32–35  :  4 B  uint32  original file size
#  36–39  :  4 B  uint32  ciphertext length
#  40     :  1 B  filename length N
#  41..   :  N B  filename (UTF-8)
#  next   :  1 B  MIME type length M
#  next M :  M B  MIME type (UTF-8)
#  next   :  1 B  compress flag  (1=zlib used, 0=raw)
...
# MIME types / prefixes that compress well with zlib
_GOOD_COMPRESS = (
...
# MIME types / prefixes whose data is already compressed — skip zlib
_SKIP_COMPRESS = (
    ...
    'application/x-bzip2',
    'application/vnd.rar',
    'application/octet-stream',     # unknown binary — store raw
)
...
def _should_compress(mime: str) -> bool:
    ...
    if m.startswith('image/'):  return True   # PNG/BMP/TIFF — try, skip if bigger
    return False
...
# probe: read this many bytes to parse the header and get total size
PROBE_BYTES = 320
...
def _total_from_probe(probe: bytes) -> tuple[int, int]:
    ...
    # magic(4) + salt(16) + nonce(12) + orig_size(4) + enc_len(4) = 40 bytes fixed
...
# ─────────────────────────────────────────────────────────────
# BUG FIX: The previous version used _safe_indices() to filter out
# near-black (rgb_sum < 24) and near-white (rgb_sum > 741) pixels
# before building the embedding permutation.
#
# This caused a critical encode/decode mismatch:
#   - During ENCODE: _safe_indices runs on the original cover image.
#   - During DECODE: _safe_indices runs on the stego image, where
#     NC-LSBM has already changed some pixel values by ±1.
#
# A pixel at exactly rgb_sum == 23 in the cover becomes rgb_sum == 24
# in the stego image (or vice versa), flipping its membership in the
# safe set. This shifts every subsequent index in the permutation,
# so the extracted bits no longer start with MAGIC_BYTES.
#
# TEXT files worked by coincidence: their payloads are small and
# likely land entirely before any threshold-crossing pixel in the
# permutation order.
#
# FIX: Use ALL pixels for embedding. A ±1 LSB change is perceptually
# invisible on any pixel value (it is a 0–100% change only on pure
# black/white, but those values are rare in natural photographs and
# the change is still only 1 grey level). The AES-GCM encrypted
# payload already looks like random noise regardless of which pixels
# carry it, so there is no steganalytic advantage to avoiding
# near-black pixels.
# ─────────────────────────────────────────────────────────────
...
def embed_payload(pixels: np.ndarray, payload: bytes, password: str) -> np.ndarray:
    ...
    # Password-keyed permutation over ALL channel indices
    seed   = int.from_bytes(
...
async def get_capacity(cover_image: UploadFile = File(...)):
    ...
    try:
        pixels, w, h = load_png(data)  # BUG FIX: was `_, w, h` — pixels was undefined below
